<template>
	<div class="common-layout">
			...
			<el-main>
				<el-row :gutter="20">
					...
					<el-col :span="15">
						<div class="card-container">
							<div class="card-header">
								<span class="card-title">外出登记</span>
							</div>
							<!-- 外出登记表格 -->
							<el-table :data="outwardList" size="small" style="width: 100%; color:black;" stripe>
		...
		<el-dialog v-model="dialog.dialogTimeVisible" :title="dialog.tops" width="40%" align-center draggable
			...
			<template #footer>
				<span class="dialog-footer" style="padding-top: 0px">
					<el-button type="primary" @click="updateBackTime('itemTimeForm')"
						class="operation-btn">保存</el-button>
					<el-button @click="cancelTime" class="operation-btn">取消</el-button>
				</span>
			</template>
		</el-dialog>
		...
		<el-dialog v-model="dialog.dialogExamineVisible" :title="dialog.tops" width="40%" align-center draggable
			...
			<template #footer>
				<span class="dialog-footer" style="padding-top: 0px">
					<el-button type="primary" @click="examineOutward('itemExamineForm')"
						class="operation-btn">保存</el-button>
			...
			<el-form label-position="right" label-width="auto" style="max-width:380px;margin:20px auto"
				...
				<el-form-item label="预计回院时间： " prop="expectedreturntime">
					<el-date-picker style="width:320px" v-model="dialog.item.expectedreturntime" type="date"
						placeholder="预计回院时间:" value-format="YYYY-MM-DD" />
				</el-form-item>
	...
	export default {
		...
		data() {
			return {
				// 模态框数据
				dialog: {
					dialogVisible: false, // 添加申请对话框状态
					dialogTimeVisible: false, // 修改时间对话框状态
